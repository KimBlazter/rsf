PROCHE_AVG_SNR = 10  # SNR for close users
LOIN_AVG_SNR = 7  # SNR for far users

LOW_RELAY_RATIO = 0.1
HIGH_RELAY_RATIO = 0.5 # Don't go beyond 0.5

PDOR_THRESHOLD = 50
PDOR_TICK_INTERVAL = 10  # how often we update PDOR, in ticks
# WFO fairness calibration from cedric.
WF_ALPHA = 3
WF_BETA = 10**6

BITS_PER_SNR_POINT = (
    80  # when allocatin a UR, how much bits do we remove from buffer per SNR point
)
MAX_UR = 640  # 128 * 5 # how much UR are available each tick
PACKET_SIZE = 50

# (moySNR * bits per snr * nb ur) / bits per user per tick
